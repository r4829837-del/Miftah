...
function NewsPanel() {
  ...
  const loadNewsFromStorage = () => {
    try {
      const savedNews = getStorage('newsData');
      const savedCheckTime = getStorage('lastNewsCheck');
      ...
      if (savedCheckTime) {
        setLastCheck(new Date(savedCheckTime));
      }
    } catch (error) {
  ...
  const saveNewsToStorage = () => {
    try {
      setStorage('newsData', news);
      setStorage('lastNewsCheck', Date.now());
    } catch (error) {
