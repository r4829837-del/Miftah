...
const reportTypes = (currentCycle: string) => [
  ...
  },
  {
    ...
    description: currentCycle === 'ثانوي' ? 'تقرير عملية الإعلام والتوجيه لأولياء الطلاب' : 'تقرير عملية الإعلام والتوجيه لأولياء التلاميذ',
    icon: FileText,
...
export default function Reports() {
  ...
  const [currentReportType, setCurrentReportType] = useState<'info' | 'parents'>('info');
  ...
  useEffect(() => {
    setReportData(prev => ({
      ...prev,
      coverageRows: prev.coverageRows.map((row) => ({
        ...row,
        group: 'جميع المستويات'
      }))
    }));
  ...
  const handleGeneratePDF = async (type: 'student' | 'parents' | 'annual' | 'activities' | 'objectives') => {
    const contentId =
    ...
    try {
      ...
      if (type === 'student') {
        ...
        data = { ...reportData, cycle: currentCycle };
        title = 'تقرير عملية اعلام الأولياء';
        typeName = 'تقرير عملية اعلام الأولياء';
        fileName = 'تقرير_عملية_اعلام_الأولياء.pdf';
  ...
  const handleReportTypeClick = (type: string) => {
    if (type === 'annual') {
      // Rapport annuel désactivé - afficher un message
      alert('التقرير السنوي قيد الانجاز');
      return;
    }
    if (type === 'info') {
      setCurrentReportType('info');
      setShowPreview(true);
    } else if (type === 'parents') {
      setCurrentReportType('parents');
      setShowPreview(true);
    } else if (type === 'objectives') {
    ...
    <div className="space-y-6">
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-7xl mx-4 max-h-[95vh] overflow-y-auto">
            ...
            <div id="report-preview" className="bg-white p-6 rounded-lg space-y-6 text-lg" dir="rtl">
              ...
              <div className="text-center my-4">
                <div className="text-2xl font-bold mb-1">
                  {currentReportType === 'info' ? 'تقرير عملية الإعلام' : 'تقرير عملية اعلام الأولياء'}
                </div>
                  ...
                  <table className="w-full border-collapse border-2 border-gray-700 text-lg">
                    ...
                    <tbody>
                      ...
                      <tr className="bg-gray-50 font-bold">
                        ...
                        <td className="border-2 border-gray-700 p-2 text-center">
                          <div className="text-center w-full">{calculateTotalCoverage()}%</div>
                        </td>
                        <td className="border-r-2 border-l-2 border-gray-700 p-2 text-center">
                          <div className="text-center w-full">-</div>
                        </td>
            ...
            <div className="flex justify-end gap-4 mt-6">
              ...
              <button
                onClick={() => handleGeneratePDF(currentReportType === 'info' ? 'student' : 'parents')}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {reportTypes(currentCycle).map((type) => (
          <div key={type.id} className={`bg-white p-6 rounded-lg shadow-md transition-shadow ${type.id === 'annual' ? 'opacity-75' : 'hover:shadow-lg'}`}>
            ...
            {type.id === 'annual' ? (
              <div className="w-full bg-gray-400 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 cursor-not-allowed">
